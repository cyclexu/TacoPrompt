...
def margin_rank_loss(output, target, sample_size=32, margin=0.2):
...
class DistMarginLoss:
    ...
    def loss(self, output, target, nodes):
        ...
        # loss = F.margin_ranking_loss(output[positive_indices,:], output[negative_indices,:], y, margin=margins, reduction="sum")
        loss = F.margin_ranking_loss(output[positive_indices], output[negative_indices], y, margin=margins, reduction="sum")
        ...
        # loss = (-output[positive_indices].clamp(min=EPS) + output[negative_indices].clamp(min=EPS)  ).clamp(min=0)
