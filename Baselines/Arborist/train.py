...
def main(config):
    ...
    spdist = train_data_loader.dataset.spdist
    # get function handles of loss and metrics
    if config['loss'].startswith("DistMarginLoss"):
        loss = getattr(module_loss, config['loss'])(spdist)
        print("DistMarginLoss")
    else:
    ...
    if config['loss'].startswith("DistMarginLoss"):
        pre_metric = partial(module_metric.obtain_ranks, mode=0)  # info_nce_loss
        # print("here")
        print("DistMarginLoss")
    if config['loss'].startswith("margin_rank_loss"):
        pre_metric = partial(module_metric.obtain_ranks, mode=0)  # info_nce_loss
        print("here")
