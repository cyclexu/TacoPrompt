...
def main(config):
    ...
    ckpt_path = config['arch']['args']['hyp_ckpt_path']
    if ckpt_path:
        # xu:保证了两个模型用同样的seed初始化，逻辑上等同于连着训练两个模型
        ckpt = torch.load(ckpt_path)
        model.load_state_dict(ckpt['state_dict'])
        for params in model.hp_scorer.parameters():
    ...
    for loss_name in config['loss']:
        loss = getattr(module_loss, loss_name)
        if loss_name.startswith("FocalLoss"):
            loss = loss()
        all_loss.append(loss)
    metrics = [getattr(module_metric, met) for met in config['metrics']]
    if not ckpt_path and config['arch']['args']['freeze_hyp']:
        pre_metric = partial(module_metric.obtain_ranks, mode=0)
    # xu:不是freeze模式/freeze模式下有ckpt
    else:
        if config['loss'][0].startswith("info_nce") or config['loss'][0].startswith("bce_loss"):
            # todO:gai hui lai
    ...
    trainer = Trainer(model, all_loss, metrics, pre_metric, optimizer,
                      config=config,
