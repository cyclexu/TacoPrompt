...
class Tester:
    def __init__(self, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None, hit_metrics=None, recall_metrics=None,rerank_dict = None):
        ...
        with open(rerank_dict,'rb') as f:
            self.retrieval_list = pickle.load(f)


    def infer(self):
        mode = 'test'
        logger = logging.getLogger("1")
        hit_total_metrics, hit_leaf_metrics, hit_nonleaf_metrics,recall_total_metrics, recall_leaf_metrics,recall_nonleaf_metrics = self._test(mode)        
        for i, mtr in enumerate(self.hit_metrics):
            logger.info('    {:15s}: {:.3f}'.format('test_' + mtr.__name__, hit_total_metrics[i]))
        for i, mtr in enumerate(self.hit_metrics):
            logger.info('    {:15s}: {:.3f}'.format('test_leaf_' + mtr.__name__, hit_leaf_metrics[i]))
        for i, mtr in enumerate(self.hit_metrics):
            logger.info('    {:15s}: {:.3f}'.format('test_nonleaf_' + mtr.__name__, hit_nonleaf_metrics[i]))
        for i, mtr in enumerate(self.recall_metrics):
            logger.info('    {:15s}: {:.3f}'.format('test_' + mtr.__name__, recall_total_metrics[i]))
        for i, mtr in enumerate(self.recall_metrics):
            logger.info('    {:15s}: {:.3f}'.format('test_leaf_' + mtr.__name__, recall_leaf_metrics[i]))
        for i, mtr in enumerate(self.recall_metrics):
            logger.info('    {:15s}: {:.3f}'.format('test_nonleaf_' + mtr.__name__, recall_nonleaf_metrics[i]))
    ...
    def _test(self, mode, gpu=True):
        ...
        with torch.no_grad():
            ...
            hit_all_ranks = []
            leaf_queries = []
            hit_leaf_ranks, hit_nonleaf_ranks = [], []
            recall_all_ranks = []
            recall_leaf_ranks, recall_nonleaf_ranks = [], []
            ...
            for i, query in tqdm(enumerate(eval_queries), desc=mode, total=len(eval_queries)):
                ...
                query2topkpos = self.retrieval_list[query_id][:TOPK]
                
                for edges in mit.sliced(query2topkpos, batch_size):
                    edges = list(edges)
                    ps, cs = zip(*edges)

                    with autocast():
                        p_idx = torch.tensor([all_node_id_2all_embed_id[n] for n in ps]).to(self.device)
                        c_idx = torch.tensor([all_node_id_2all_embed_id[n] for n in cs]).to(self.device)
                ...
                # ranks for hit metric
                grandtruth_query2pos = [(taxon2all_node_id[pos[0]],taxon2all_node_id[pos[1]]) for pos in node2pos[query]]
                
                topkgrandtruth = []
                for item in grandtruth_query2pos:
                    if item in query2topkpos:
                        topkgrandtruth.append(item)
                
                batched_energy_scores_cat_hit, labels = rearrange(batched_energy_scores_cat, query2topkpos,
                                                              topkgrandtruth)
                ranks = self.pre_metric(batched_energy_scores_cat_hit, labels)
                hit_all_ranks.extend(ranks)

                if query in leaf_queries:
                    hit_leaf_ranks.extend(ranks)
                else:
                    hit_nonleaf_ranks.extend(ranks)
                
                # ranks for recall metric
                np_candidate_positions = [[taxon2all_node_id[pos[0]],taxon2all_node_id[pos[1]]] for pos in candidate_positions]
                np_candidate_positions = np.array(np_candidate_positions)
                np_querytopkpos = np.array(query2topkpos).tolist()
                res_idx = [np.argwhere((np_candidate_positions == x).all(axis=1)).flatten() for x in np_querytopkpos]
                res_idx = torch.tensor(res_idx).long().flatten()
                batched_energy_scores_all = torch.tensor([-100000] * len(candidate_positions),dtype=torch.float16).to(self.device)
                batched_energy_scores_all[res_idx] = batched_energy_scores_cat
                batched_energy_scores_cat_recall, labels = rearrange(batched_energy_scores_all, candidate_positions,
                                                              node2pos[query])
                ranks = self.pre_metric(batched_energy_scores_cat_recall, labels)
                recall_all_ranks.extend(ranks)

                if query in leaf_queries:
                    recall_leaf_ranks.extend(ranks)
                else:
                    recall_nonleaf_ranks.extend(ranks)

            # hit metric
            hit_total_metrics = [metric(hit_all_ranks) for metric in self.hit_metrics]
            hit_leaf_metrics = [metric(hit_leaf_ranks) for metric in self.hit_metrics]
            hit_nonleaf_metrics = [metric(hit_nonleaf_ranks) for metric in self.hit_metrics]
            
            # recall metric
            recall_total_metrics = [metric(recall_all_ranks) for metric in self.recall_metrics]
            recall_leaf_metrics = [metric(recall_leaf_ranks) for metric in self.recall_metrics]
            recall_nonleaf_metrics = [metric(recall_nonleaf_ranks) for metric in self.recall_metrics]

        return  hit_total_metrics, hit_leaf_metrics, hit_nonleaf_metrics,recall_total_metrics, recall_leaf_metrics,recall_nonleaf_metrics
...
if __name__ == '__main__':
    ...
    args.add_argument('-rl', '--retrieval_list', default=None, type=str, help='topk list in the first stage (default: None)')
    ...
    rerank_dict = args.retrieval_list
    assert rerank_dict != None,"Please enter retrieval list!"
    ...
    tester = Tester(model, loss, metrics, pre_metric, optimizer=None, config=config, data_loader=test_data_loader, lr_scheduler=None,hit_metrics=hit_metrics,recall_metrics=recll_metrics,rerank_dict=rerank_dict)
    tester.infer()
