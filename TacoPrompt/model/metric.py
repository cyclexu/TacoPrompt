...
def mrr(all_ranks):
    ...
    """
    rank_positions = np.array(list(itertools.chain(*all_ranks)))
    return (1.0 / rank_positions).mean()

def combined_metrics(all_ranks):
    """ 
    combination of three metrics, used in early stop 
    """
    score = macro_mr(all_ranks) * (1.0 / max(mrr_scaled_10(all_ranks), 0.0001)) * (
                1.0 / max(recall_at_3(all_ranks), 0.0001)) * (1.0 / max(recall_at_1(all_ranks), 0.0001))
    return score
